...
import { useGetClassroomQuery, useUpdateClassroomMutation, useGetClassroomMembersQuery } from '@/services/classesApi'
...
export default function ClassManagementPage({
  ...
  const { data: cls, isLoading: classLoading } = useGetClassroomQuery(classId)
  const { data: teachers = [] } = useGetClassroomMembersQuery(
    { classId, role: 'TEACHER' },
  )
  const [updateClassroom, { isLoading: updating }] = useUpdateClassroomMutation()

  async function handleEdit(values: { name: string; description: string }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border bg-surface-raised p-6 border-surface-border">
        ...
        <StudentEnrollmentPanel classId={classId} />
      </div>
