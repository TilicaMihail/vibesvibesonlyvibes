...
export default function CourseStudyPage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return (
    <div className="flex h-full -m-6">
      {/* Left sidebar */}
      <div className="w-72 shrink-0 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-1 font-medium truncate">{course?.title}</p>
        ...
        {!selectedNode ? (
          ...
          <div className="max-w-3xl mx-auto">
            <div className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500 mb-4">
              ...
              <span>/</span>
              <span className="text-gray-700 dark:text-gray-300">{selectedNode.title}</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">{selectedNode.title}</h1>

            {selectedNode.type === 'text' && (
              <div className="prose prose-sm max-w-none bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 whitespace-pre-wrap font-mono text-sm text-gray-700 dark:text-gray-300 leading-relaxed">
            ...
            {selectedNode.type === 'file' && (
              <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 flex items-center gap-4">
                <div className="w-12 h-12 bg-indigo-100 dark:bg-indigo-900/30 rounded-lg flex items-center justify-center text-2xl">📎</div>
                <div>
                  <p className="font-medium text-gray-900 dark:text-gray-100">{selectedNode.fileName ?? 'File'}</p>
                  <a href={selectedNode.fileUrl ?? '#'} className="text-sm text-indigo-600 dark:text-indigo-400 hover:underline">Download</a>
                </div>
              </div>
            )}

            {selectedNode.type === 'chapter' && (
              <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl p-6 text-indigo-800 dark:text-indigo-200">
            ...
            {nodes.filter(n => n.type !== 'chapter').length > 0 && (
              <div className="mt-8 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
                <h3 className="font-semibold text-gray-800 dark:text-gray-200 mb-3">🧠 Generate Custom Test</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">Select topics to test yourself on</p>
                <div className="flex flex-wrap gap-2 mb-4">
                  {nodes.filter(n => n.parentId !== null && n.type !== 'test').map(n => (
                    <button
                      key={n.id}
                      onClick={() => toggleTopic(n.title)}
                      className={`cursor-pointer text-xs px-3 py-1.5 rounded-full border transition-colors ${selectedTopics.includes(n.title) ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:border-indigo-400 dark:hover:border-indigo-500'}`}
                    >
                      {n.title}
                    </button>
                  ))}
                ...
                <Link
                  ...
                  className={`inline-block text-sm font-medium px-4 py-2 rounded-lg transition-colors ${selectedTopics.length > 0 ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-400 dark:text-gray-500 pointer-events-none'}`}
                >
                  Generate Test
                </Link>
              </div>
