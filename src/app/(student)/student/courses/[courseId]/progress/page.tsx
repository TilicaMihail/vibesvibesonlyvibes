...
import { useGetCourseProgressQuery, useGetUserProgressQuery } from '@/services/progressApi';
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
        ...
        <span>/</span>
        <Link href={`/student/courses/${courseId}/study`} className="hover:text-indigo-600">{course?.title}</Link>
        <span>/</span>
        <span className="text-gray-700">Progress</span>
      </div>

      <h1 className="text-2xl font-bold text-gray-900 mb-6">Course Progress</h1>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Completion', value: `${pct}%`, color: pct >= 80 ? 'text-green-600' : 'text-indigo-600' },
          { label: 'Resources Done', value: `${completedIds.size}/${resourceNodes.length}`, color: 'text-gray-900' },
          { label: 'Tests Taken', value: testSessions, color: 'text-gray-900' },
          { label: 'Last Access', value: progress?.lastAccessedAt ? new Date(progress.lastAccessedAt).toLocaleDateString() : '—', color: 'text-gray-500' },
        ].map(kpi => (
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-200">
          <h2 className="text-sm font-semibold text-gray-700">Resource Completion</h2>
        </div>
          ...
          {resourceNodes.map(node => {
            ...
            return (
              <div key={node.id} className="flex items-center gap-3 px-4 py-3">
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${done ? 'border-green-500 bg-green-500' : 'border-gray-300'}`}>
                  {done && <span className="text-white text-xs">✓</span>}
                </div>
                <span className="text-sm text-gray-800 flex-1">{node.title}</span>
                <Badge variant={node.type === 'test' ? 'warning' : node.type === 'video' ? 'info' : 'neutral'}>{node.type}</Badge>
                {rp?.completedAt && <span className="text-xs text-gray-400">{new Date(rp.completedAt).toLocaleDateString()}</span>}
              </div>
            );
          })}
        </div>
