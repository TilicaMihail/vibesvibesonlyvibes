...
export default function TestResultsPage({ params }: { params: Promise<{ courseId: string; sessionId: string }> }) {
  ...
  if (!session) return <div className="text-center py-20 text-gray-400">Results not found</div>;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href={`/student/courses/${courseId}/study`} className="text-sm text-indigo-600 hover:underline">← Back to course</Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Incorrect', value: total - correct, color: 'text-red-600' },
        ].map(s => (
      ...
      <div className="space-y-3">
        <h2 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">Question Review</h2>
        {session.answers.map((answer, i) => {
          ...
          return (
            <div key={answer.questionId} className={`bg-white border rounded-xl p-4 ${correct_q ? 'border-green-200' : 'border-red-200'}`}>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="space-y-1 text-xs">
                    <p className={correct_q ? 'text-green-600' : 'text-red-600'}>
                      Your answer: <strong>{yourOptions || 'Not answered'}</strong>
                    </p>
                    {!correct_q && <p className="text-green-600">Correct: <strong>{correctOptions}</strong></p>}
                    {question.explanation && <p className="text-gray-400 italic mt-1">{question.explanation}</p>}
                  </div>
                </div>
      ...
      <div className="mt-6 flex gap-3">
        ...
        <Link href={`/student/courses/${courseId}/study`} className="flex-1 text-center py-2.5 border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-lg text-sm font-medium transition-colors">
          Back to Course
        </Link>
      </div>
      ...
      <Modal isOpen={reportOpen} onClose={() => setReportOpen(false)} title="Report Question" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">What's wrong with this question?</p>
          ...
          <div className="flex gap-2 justify-end">
            ...
            <Button variant="danger" onClick={() => setReportOpen(false)}>Submit Report</Button>
          </div>
        </div>
      </Modal>
    </div>
