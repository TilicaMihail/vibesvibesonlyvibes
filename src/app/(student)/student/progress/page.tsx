...
export default function MyProgressPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">My Progress</h1>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Active Courses', value: activeCourses, color: 'text-indigo-600 dark:text-indigo-400' },
          ...
          { label: 'Total Enrolled', value: courses.length, color: 'text-gray-900 dark:text-gray-100' },
        ].map(s => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700">
            <h2 className="font-semibold text-gray-800 dark:text-gray-200">Course Progress</h2>
          </div>
                ...
                return (
                  <div key={p.id} className="px-5 py-4">
                    <div className="flex justify-between items-center mb-2">
                      <Link href={`/student/courses/${p.courseId}/progress`} className="text-sm font-medium text-gray-800 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 truncate max-w-[70%]">
                        {course?.title ?? p.courseId}
                      </Link>
                      <span className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{p.completionPercent}%</span>
                    </div>
                    <ProgressBar value={p.completionPercent} size="sm" color={p.completionPercent === 100 ? 'green' : 'indigo'} />
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">Last accessed {new Date(p.lastAccessedAt).toLocaleDateString()}</p>
                  </div>
                );
              })}
            </div>
              ...
              {activity.slice(0, 10).map(a => (
                <div key={a.id} className="px-5 py-3 flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-700 dark:text-gray-300">{activityText(a)}</p>
                    {a.testSessionId && (
                      <Link href={`/student/courses/${a.courseId}/test/${a.testSessionId}/results`} className="text-xs text-indigo-500 dark:text-indigo-400 hover:underline">View results</Link>
                    )}
                  </div>
                  <span className="text-xs text-gray-400 dark:text-gray-500 shrink-0">{new Date(a.timestamp).toLocaleDateString()}</span>
                </div>
              ))}
            </div>
          )}
