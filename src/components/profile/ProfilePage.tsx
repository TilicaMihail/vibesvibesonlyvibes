      ...
      <Card padding="lg">
        ...
        {user.role === 'student' && (
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center"><div className="text-2xl font-bold text-indigo-600">{studentCourses.length}</div><div className="text-xs text-gray-500">Courses</div></div>
            <div className="text-center"><div className="text-2xl font-bold text-indigo-600">{totalTests}</div><div className="text-xs text-gray-500">Tests Taken</div></div>
            <div className="text-center"><div className="text-2xl font-bold text-green-600">{progressList.filter(p => p.completionPercent === 100).length}</div><div className="text-xs text-gray-500">Completed</div></div>
          </div>
        )}
        {user.role === 'teacher' && (
          <div className="grid grid-cols-2 gap-4">
            <div className="text-center"><div className="text-2xl font-bold text-indigo-600">{courses.length}</div><div className="text-xs text-gray-500">Courses Created</div></div>
            <div className="text-center"><div className="text-2xl font-bold text-indigo-600">{courses.reduce((sum, c) => sum + c.enrolledStudentIds.length, 0)}</div><div className="text-xs text-gray-500">Total Students</div></div>
          </div>
        )}
        {user.role === 'admin' && (
          <div>
            <p className="text-sm text-gray-600"><span className="font-medium">Organization:</span> {org?.name ?? '—'}</p>
            <p className="text-sm text-gray-600 mt-1"><span className="font-medium">Slug:</span> {org?.slug ?? '—'}</p>
          </div>
        )}
      </Card>
    </div>
