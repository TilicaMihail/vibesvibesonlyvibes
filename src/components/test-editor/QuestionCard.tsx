    ...
    <div className="bg-surface-raised border border-surface-border rounded-lg p-4 space-y-2">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2 flex-1">
          ...
          <p className="text-sm text-on-surface font-medium">{question.text}</p>
        </div>
      ...
      <div className="pl-8 space-y-1">
        {question.options.map(opt => (
          <div key={opt.id} className={`flex items-center gap-2 text-xs rounded px-2 py-1 ${opt.isCorrect ? 'bg-green-50 text-green-700' : 'text-on-surface-muted'}`}>
